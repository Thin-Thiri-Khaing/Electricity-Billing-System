...
//		pw.println("<script language='text/javascript' src>")
		...
		pw.println("#successRemove{text-align : center; color : #00BB27; margin-top : 2vh;}");
		pw.println("#error{color : red; width:100%; text-align : center; margin-top : 2vh;}");
		...
		pw.println("<div id='searchInput'");
		pw.println("<span><i class='fa-solid fa-search'></i></span>");
		pw.println(" <input type='text' name='mbi' id='textbox' placeholder='Search MeterBox ID' required' class='textinput1'>");
		...
		try {
			...
			PreparedStatement st = con.prepareStatement("delete from users where meterbox_id=?");
			st.setString(1, mbi);
			int i = st.executeUpdate();
			if(i>0) {
				pw.println("<h3 id='successRemove'><i class='fa-solid fa-check'></i>&nbsp;&nbsp;User is successfully removed!</h3>");
			}
			
			else {
				pw.println("<h3 id='error'><i class='fa-solid fa-triangle-exclamation'></i>&nbsp;&nbsp;There is no Meterbox ID you want to delete!</h3>");
			}
